...
void __not_in_flash_func(ComputerCard::AudioWorker)()
{
	...
	// LENS EDIT: PWM IRQ priority raised above audio DMA so CV doesn't freeze under load.
...
void __not_in_flash_func(ComputerCard::BufferFull)()
{
	...
	// LENS EDIT: conditional ADC round-robin resync (only on overrun) to preserve
	// 48 kHz pacing. Pattern after Vincent Maurer's Grains card (Workshop System #51).
	bool adc_desynced = adc_fifo_get_level() != 0 || (adc_hw->fcs & ADC_FCS_OVER_BITS);
	if (adc_desynced)
	{
		adc_run(false);
		adc_fifo_drain();                                           // discard overrun leftovers
		adc_hw->fcs |= ADC_FCS_OVER_BITS;                           // write-1-to-clear
	}
	...
	if (adc_desynced)
	{
		...
		adc_select_input(0);
		adc_set_round_robin(0b0001111U);
		adc_run(true);                                              // restart: clean FIFO, known channel
	}
